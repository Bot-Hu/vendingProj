...
//size:字体大小
//mode:0,正常显示,1,叠加显示	   
void Show_Font_Color(u16 x,u16 y,u8 *font,u8 size,u8 mode,u16 color)
{
	u8 temp,t,t1;
	u16 y0=y;
	u8 dzk[72];   
...
//size :字体大小
//mode:0,非叠加方式;1,叠加方式    	   		   
void Show_Str_Color(u16 x,u16 y,u16 width,u16 height,u8*str,u8 size,u8 mode,u16 color)
{					
	u16 x0=x;
	u16 y0=y;							  	  
    u8 bHz=0;     //字符或者中文  	    				    				  	  
    ...
    { 
        ...
        {     
	        ...
	        if(y>(y0+height-size))break;//越界返回  						     
	        Show_Font_Color(x,y,str,size,mode,color); //显示这个汉字,空心显示 
	        str+=2; 
	        x+=size;//下一个汉字偏移	    
        }						 
    }   
...
void PutHZ16(u16 x,u16 y,u8 c[],u8 mode)
{
	...
	for(n=0;n<HZ_NUM;n++) //HZ_NUM标示自建汉字库中的个数，循环查询内码
	{
		if((codeHZ_16[n].Index[0]==c[0])&&(codeHZ_16[n].Index[1]==c[1]))
		{
			for(t=0;t<32;t++)	//	16*16共32个数据
			{
				temp=codeHZ_16[n].Msk[t];			//得到点阵数据                          
				for(t1=0;t1<8;t1++)
				{
					if(temp&0x80)		LCD_Fast_DrawPoint(x,y,POINT_COLOR);
					else if(mode==0)	LCD_Fast_DrawPoint(x,y,BACK_COLOR); 
...
void Show_HZ16_Color(u16 x,u16 y,u8*str,u8 mode,u16 color)	 
{
	while(*str)
	{
		PutHZ16_Color(x,y,str,mode,color);
		str+=2;
		x+=16;
	}
}
